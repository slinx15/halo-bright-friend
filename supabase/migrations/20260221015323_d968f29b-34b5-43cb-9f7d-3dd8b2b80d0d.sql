...
-- Drop restrictive policies and recreate as permissive for products, prices, stock
...
CREATE POLICY "Admin can manage prices" ON public.prices
  FOR ALL USING (has_role(auth.uid(), 'admin'::app_role))
  WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "Authenticated can read prices" ON public.prices
  FOR SELECT USING (true);
