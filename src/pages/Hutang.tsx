  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] space-y-4 p-4 pb-[calc(9rem+env(safe-area-inset-bottom))] md:space-y-5 md:p-6 md:pb-6">
      <PageHeader
        ...
        title="Hutang Ivory"
        subtitle="Kelola limit supplier, bon lunas, dan pembayaran manual"
        actions={
          <div className="flex w-full flex-col gap-2 sm:flex-row">
            <div className="flex items-center gap-2 rounded-xl border border-border/70 bg-card px-3 py-2">
              ...
              <Input
                ...
                className="h-9 w-32 rounded-lg text-sm font-semibold"
              />
            </div>
        ...
        <CardContent className="space-y-3 px-4 pb-4 pt-1">
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_220px]">
            <Textarea
              ...
              placeholder="Catatan, misalnya: kuning / lunas / jatuh tempo"
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className={cn("mt-1 text-xs", item.note.toLowerCase().includes("lunas") ? "text-warning font-semibold" : "text-muted-foreground")}>
                      {item.note || "Tanpa catatan"}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-base font-extrabold tabular-nums">{formatRupiah(item.amount)}</p>
                    <p className="text-xs text-muted-foreground">{isSelected ? "dipilih" : "ketuk untuk pilih"}</p>
                  </div>
                </div>
              </button>
            );
      ...
      <Card className="rounded-2xl border bg-card shadow-sm">
        ...
        <CardContent className="space-y-2 px-4 pb-4 pt-1">
          ...
          {paidItems.map((item) => (
            <div key={item.id} className="rounded-2xl border border-warning/30 bg-warning/5 p-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="font-mono text-sm font-bold">{item.invoiceNumber}</p>
