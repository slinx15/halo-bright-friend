  ...
  return (
    <div className="mx-auto w-full max-w-[1400px] space-y-4 p-4 pb-[calc(9rem+env(safe-area-inset-bottom))] md:space-y-5 md:p-6 md:pb-6">
      ...
      <Card className="card-premium overflow-hidden rounded-2xl">
        ...
        <CardContent className="space-y-2 px-4 pb-4 pt-1">
          ...
          {openItems.map((item) => {
            ...
            const parsed = parseNoteItems(item.note);
            return (
              <div
                key={item.id}
                className={cn("w-full rounded-2xl border transition-all", isSelected ? "border-primary bg-primary/5 shadow-sm" : "border-border/70 bg-card")}
              >
                <button onClick={() => toggleSelect(item.id)} className="w-full p-3 text-left">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      >
                        {parsed.label || (item.note ? item.note : "Tanpa catatan")}
                        {parsed.rows.length > 0 ? ` · ${parsed.rows.length} barang` : ""}
                      </p>
                    </div>
                ...
                {parsed.rows.length > 0 && (
                  <div className="border-t border-border/60 px-3 py-2">
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-9 w-full justify-center rounded-xl text-xs font-semibold text-primary"
                      onClick={() => setDetailItem(item)}
                    >
                      <List className="mr-1.5 h-3.5 w-3.5" />
                      Lihat barang yang dibeli
                    </Button>
                  </div>
                )}
              </div>
      ...
      <Dialog open={!!detailItem} onOpenChange={(open) => !open && setDetailItem(null)}>
        <DialogContent className="max-h-[85svh] w-[calc(100vw-1.5rem)] overflow-hidden rounded-3xl p-0 sm:max-w-md">
          <DialogHeader className="border-b border-border/60 px-4 py-4 text-left">
            ...
            <DialogDescription className="text-xs">
              {detailItem?.invoiceNumber} · {detailItem?.invoiceDate} · {formatRupiah(detailItem?.amount ?? 0)}
            </DialogDescription>
          </DialogHeader>
          <div className="max-h-[60svh] overflow-y-auto px-4 py-3">
            {detailItem && parseNoteItems(detailItem.note).label && (
              <p className="mb-2 text-xs font-semibold text-muted-foreground">{parseNoteItems(detailItem.note).label}</p>
            )}
            <div className="divide-y divide-border/60 rounded-2xl border border-border/60">
              {(detailItem ? parseNoteItems(detailItem.note).rows : []).map((row, idx) => (
                <div key={`${row.kode}-${idx}`} className="flex items-center justify-between gap-3 px-3 py-2.5">
                  <div className="flex min-w-0 items-center gap-2">
                    <span className="w-6 shrink-0 text-[11px] tabular-nums text-muted-foreground">{idx + 1}.</span>
                    <span className="truncate font-mono text-sm font-semibold">{row.kode}</span>
                  </div>
                  <span className="shrink-0 text-sm font-bold tabular-nums">{formatNumber(row.qty)} pcs</span>
                </div>
              ))}
            </div>
            {detailItem && (
              <div className="mt-3 flex items-center justify-between rounded-2xl bg-muted/50 px-3 py-2.5 text-sm font-bold">
                <span>Total</span>
                <span className="tabular-nums">
                  {formatNumber(parseNoteItems(detailItem.note).rows.reduce((sum, row) => sum + row.qty, 0))} pcs
                </span>
              </div>
            )}
          </div>
